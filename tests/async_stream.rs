...
use aqueduct::{
    AsyncRetention, BlockFlags, BundleAsyncReader, BundleAsyncWriter, BundleBuilder, BundleReader,
    CanonicalBlock, Crc, Eid, HopCount, MemoryRetention, Retention,
};
...
impl AsyncRetention for AsyncMemoryRetention {
    ...
    async fn reader(&self, offset: u64, len: u64) -> std::io::Result<Self::Reader<'_>> {
        self.0.reader(offset, len)
    }
...
async fn async_writer_roundtrip() {
    ...
    let primary = aqueduct::PrimaryBlock {
        ...
        rpt_eid: Eid::Null,
        creation_ts: aqueduct::CreationTimestamp { time: 100, seq: 0 },
        lifetime: 1000,
        fragment: None,
    };
